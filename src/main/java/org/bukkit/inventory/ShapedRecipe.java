...
public class ShapedRecipe implements Recipe {
    ...
    public ShapedRecipe shape(final String... shape) {
        org.apache.commons.lang3.Validate.notNull(shape, "Must provide a shape");
        org.apache.commons.lang3.Validate.isTrue(shape.length > 0 && shape.length < 4, "Crafting recipes should be 1, 2, 3 rows, not ", shape.length);

        for (String row : shape) {
            org.apache.commons.lang3.Validate.notNull(row, "Shape cannot have null rows");
            org.apache.commons.lang3.Validate.isTrue(row.length() > 0 && row.length() < 4, "Crafting rows should be 1, 2, or 3 characters, not ", row.length());
        }
    ...
    public ShapedRecipe setIngredient(char key, Material ingredient, int raw) {
        org.apache.commons.lang3.Validate.isTrue(ingredients.containsKey(key), "Symbol does not appear in the shape:", key);
